...
功能模組（SidebarAutoHide、ChatWidth、InputWidth、HideThinking、GoToTop、QuoteReply、PreventAutoScroll、WebsearchToggle、MobileSidebarSwipe、TemporaryChatToggle、AutoRetry）透過 `content/feature-toggle.js` 中的共用管線處理開關控制：`registerFeatureToggle({ ownKey, onEnable, onDisable })`。管線以單次 `DSS_GET_SETTINGS` 往返取得初始值，並透過 `DSS_SETTINGS_CHANGED` 廣播保持最新，所有已註冊的功能共用一個 `chrome.runtime.onMessage` 監聽器。當主開關 `isEnabled !== false` **且**其自身鍵 `!== false` 時，功能即為開啟——未設定的自身鍵視為開啟，只有明確的 `false` 才會關閉。未帶 `ownKey` 註冊的模組（例如 GoToTop）僅受主開關控制。初始取得失敗時，管線讓功能保持休眠，而不是猜測為開啟。
