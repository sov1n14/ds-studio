...
> - `presets`：提示詞 CRUD（`savePromptPresets`／`saveOnePromptPreset`／`saveCurrentPresetContent`）與對話綁定。
> - `chatmap.diff`：chatPresetMap 差異計算與套用（`_applyChatPresetMapDiff`），定義 `CHUNK_SOFT_LIMIT_BYTES`（7168）。
> - `chatmap`：chatPresetMap 分塊讀寫（`bindChatToPreset`／`unbindChat`／`mutateChatPresetMap`／`getChatPresetMap`）與寫入佇列。
...
> - `init`：`initialize()` 與 `_installChunkCacheInvalidator` 的安裝呼叫。
