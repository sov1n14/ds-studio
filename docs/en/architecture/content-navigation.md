# Navigation & Interface Module Architecture

> 📂 [DS studio Documentation](../) › [Architecture](../../ARCHITECTURE.md) (Chinese) › [Content Script Modules](CONTENT_SCRIPTS.md) › Navigation & Interface
>
> **Related Specifications**: [Popup UI Specifications](../../spec/02-popup-ui.md) (Chinese) · [Prompt System Specifications](../../spec/01-prompt-system.md) (Chinese)
...
The width the overlay occupies hugs the **widest** candidate label rather than the currently displayed one. `content/preset-dropdown.width.js` builds the candidate list from every option name plus the placeholder text, measures each by temporarily writing it into the real label span and reading `scrollWidth` (so the measurement carries the label's own computed font). During measurement the label's inline `flex` is set to `none`, so `scrollWidth` reflects the text's own width rather than the flex-stretched box that depends on the overlay's previous inline width; the natural width is therefore the widest option name plus the fixed trigger chrome and stays constant across preset switches. It restores the original text and `flex` in a `finally`, and feeds the widths to the pure `pickNaturalWidth()` in `preset-dropdown.position.js`. That function returns `max(labelWidths) + arrowWidth + paddingLeft + paddingRight + gap` (`gap` is the trigger's computed value, a measured 0px used as-is; only an unparseable value such as empty or `normal` falls back to 4px), floored at `minWidth` (80) and deliberately **uncapped** — capping to the available horizontal space stays with `computePlacement()`, which clamps to `maxWidth` (200) and, in gap mode, to the measured gap.
...
4. `scheduleFindAndMount()` — a 150 ms debounced re-mount (`FIND_AND_MOUNT_DEBOUNCE_MS`), re-triggering `findAndMount()` when SPA navigation replaces the title bar. The controller owns no `MutationObserver` of its own: `content/content-script.js` holds the single `document.body` `childList + subtree` observer for the whole content layer and fans its callback out to the navigation check and to this method, each keeping its own debounce semantics. New body-level detection needs attach to that fan-out — a second body observer is not permitted.
...
**Bidirectional Sync**:
- **Overlay → Popup**: `onSelectChange(newId)` calls `StorageManager.saveActivePresetId(newId)`. On a chat with a UUID it optimistically publishes the new `chatPresetMap` in memory and persists it through the service worker with `StorageManager.bindChatToPreset(uuid, newId)` (or `unbindChat(uuid)` when the empty option is chosen), then adopts the stored map. When that write is rejected (including a missing service-worker response or `{ok:false}`), it re-reads the stored map, rolls the overlay back to the stored binding with `updateActiveId(storedId)`, calls `updatePromptPrefixFromBinding()` again, and calls `saveActivePresetId(storedId)` so the failed choice does not stay persisted as `activePresetId`; the displayed preset and the injected result stay in agreement. Each `updatePromptPrefixFromBinding()` call takes a generation number; after awaiting `StorageManager.getSettings()`, a call superseded by a newer one discards its result without writing `state.promptPrefix` (latest call wins), so a late-finishing recompute from the optimistic choice cannot overwrite the rolled-back prefix. Without a UUID it sets `pendingPresetId` instead. The popup reads these values from storage on open.
...
- The menu (`<ul id="dss-preset-menu">`) has `role="listbox"` and `aria-label="Prompt Group List"`.
...
- **Global prompt interaction**: The global prompt (if set) is still injected even when the empty preset is selected — only per-preset injection is skipped. (v4.20.0) With no displayed preset there is no per-preset `globalPromptEnabled` to read, so the toggle resolves to the legacy device-level key.
- **Re-resolution on navigation** (v4.20.0): SPA navigation does not fire `chrome.storage.onChanged`, so `handleChatChange()` recomputes `isGlobalPromptEnabled` on every branch — bound, stale binding, auto-bind, unbound existing chat, and brand-new chat. The effective preset id feeding that computation comes from `resolveOverlayPresetId()`, the same precedence helper the floating overlay uses. Without this, navigating away from a chat bound to a preset with the flag off would leave the stale flag in effect until the page was reloaded.
...
- **DOM**: A `<span id="saveStatus" class="status-hidden">Saved</span>` element next to the title in the popup header.
...
- **API** (`popup.js`): `refreshSyncStatus()` — calls `StorageManager.isSyncedWithCloud()` and `StorageManager.hasOversizedItems()` (v4.8.2), toggles the `.synced` or `.unsynced` CSS class, and sets the text to "Cloud Synced" (green), "Not Synced" (red), or "Too Large — Local Only" (red, oversized).
...
- **API** (`popup.js`): `Toast.show(message, durationMs?)` — unhides the toast, sets its text, applies `opacity: 1`, then after `durationMs` (default 2000ms) sets `opacity: 0` and hides it (400ms CSS transition delay). Used for:
  - **Export failure**: "Export failed, please refresh the page and try again" displayed for 2 seconds when `chrome.tabs.sendMessage` for Markdown export fails.
  - **JSON export success**: "Settings exported successfully" displayed for 2 seconds.
  - **JSON import success**: "Settings restored successfully. Please refresh the page." displayed for 2 seconds (followed by page reload at 3s).
  - **Sync resolution success**: "Data has been merged and synced successfully" displayed for 2 seconds (followed by page reload at 1s).
- **Backup/restore operations**: "Settings exported successfully", "Export failed", "Settings restored successfully. Please refresh the page.", "Restored Messages backup exported successfully", "Restored Messages backup imported successfully", "All Restored Messages cleared", "Clear failed" — all driven by `popup/popup.backup-manager.js`.
