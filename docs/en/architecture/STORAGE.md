...
| `isAutoRetryEnabled` | boolean | `false` | (v4.35.0) Whether auto retry is active. When true, the shared round loop in `content/auto-retry.js` clicks the page's retry button after each round's random delay. Gated by the master switch; when the key is unset, `DSS_GET_SETTINGS` fills it as `false` from `DEFAULTS`. |
| `isAutoContinueEnabled` | boolean | `false` | (v4.35.0) Whether auto continue generating is active. When true, the same round loop clicks the page's continue-generating button after each round's random delay. Gated by the master switch; when the key is unset, `DSS_GET_SETTINGS` fills it as `false` from `DEFAULTS`. |
