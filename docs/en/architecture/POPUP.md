# Popup and Editor Architecture

> 📂 [DS studio Docs](../) › [Architecture](../ARCHITECTURE.md) › Popup and Editor
>
> **Related specs**: [Popup UI Spec](../spec/02-popup-ui.md) · [Prompt System](../spec/01-prompt-system.md)
...
- Inline action buttons: a pin toggle and delete (`✕`), both rendered inside each dropdown row, plus a standalone `+` button for adding new presets. Renaming happens inside the standalone editor window; the pin toggle (v4.18.0) sits to the left of `✕`.

**Add flow**: Click `+` → `Modal.prompt('Add Prompt Group')` with required-field validation → user enters name → preset created with empty content → auto-selected → if on a bound conversation, updates the binding → user edits content in textarea.

**Rename flow (v4.14.0)**: Renaming happens in the standalone editor window. Click `#editPresetBtn` → the editor opens with the preset name shown in a focused, selected name input (`#editorNameInput`) in the header → editing the name joins the same auto-save pipeline as the content (500 ms debounce + flush on blur/close). Duplicate names are rejected at save time (`DUPLICATE_NAME`) and surfaced as a red error in the save-status area; the save is blocked until the name is unique. The open popup's dropdown refreshes automatically via live-sync. (Pre-v4.14.0 the flow was: click `✎` in the dropdown row → `Modal.prompt('Rename', { value: currentName })`.)
...
**DOM structure**: A `position: fixed` overlay (with semi-transparent background) covers the entire popup area, centered vertically and horizontally. The dialog box contains a title, optional message, optional input field with a `* Required` validation indicator, and action buttons.

**API**:
- `Modal.prompt({ title, value?, placeholder? })` → `Promise<string | null>` — Shows an input dialog. The confirm button is disabled when the input is empty, and a red `* Required` indicator appears. Overlay click does NOT dismiss; only Cancel button or Escape key closes.
...
| `blankItemEl` | `Element` | The static "(No Prompt Group)" blank option element. |
| `emptyHintEl` | `Element` | The "No Results Found" empty-search hint element. |
...
- **`_updateTrigger()`**: Reads the active preset ID, finds its name in the presets list, and sets `valueEl.textContent`. Falls back to "(No Prompt Group)" when no preset is active.
...
- **`_renderList()`**: Rebuilds the dropdown list DOM. Iterates presets, filters by `filteredIds`, and creates `div.ds-select__item` elements with three child regions:
  ...
  - Inline action buttons (pin, class `ds-select__item-btn--pin`, left of delete, class `ds-select__item-btn--delete`), carrying an i18n `title`/`aria-label` (delete: "Delete prompt"; renaming happens in the editor window)
...
Note: `_fuzzyMatch()` is a private module-level function in `custom-select.js`. `test/unit/popup-utils.fuzzy-debounce.spec.js` and `test/unit/popup-custom-select.spec.js` exercise it by eval-loading the shipped `custom-select.js`.
...
The static "(No Prompt Group)" blank item (`blankItemEl`) now also renders a `.ds-select__item-btn--delete-all` button (`✕` glyph, i18n title "Delete all prompt groups"), positioned like the per-row delete buttons and revealed on hover via the same `.ds-select__item--empty:hover .ds-select__item-btn` rule in `popup-select.css`. The panel click handler in `_bindEvents()` checks for this button class before the blank-option selection branch, calls `onRequestDeleteAll()`, and returns early (stopping propagation) so the click never also fires `onSelect('')`. `popup.preset-manager.js`'s `requestDeleteAllPresets()` mirrors `requestDeletePreset(id)`: it confirms via `Modal.confirm({ title: 'Delete All Prompt Groups', message: 'Are you sure you want to delete all prompt groups? This action cannot be undone.', variant: 'danger' })`, then on confirm clears `presets` to `[]`, resets `activePresetId` to `''`, strips every `chatPresetMap` entry that referenced any of the deleted preset ids (not just one, unlike the single-delete path), and calls `StorageManager.savePromptPresets([])` — reusing the existing tombstone/cleanup machinery in `storage-manager.presets.js`, since it diffs the old vs. new `PRESET_INDEX` to compute `deletedIds`.
...
- `utils/tab-control.js` registers `globalThis.DSSTabControl`, whose `broadcastActivePreset(presetId, presetContent)` is used by `popup.js` and `editor.storage.js` for the `ACTIVE_PRESET_CHANGED` broadcast. It is a true broadcast: it queries **every** open `chat.deepseek.com` tab and sends the message to all of them concurrently via `Promise.all`, rather than only the active tab. Tabs without an `id` are skipped, and each `sendMessage` rejection is swallowed per tab — a tab whose content script has not yet loaded (or is mid-navigation) cannot stop the remaining tabs from receiving the update, and the caller's `await` always resolves.
- Three locale scripts load in sequence: `utils/i18n.locales.zhTW.js` registers the `zh_TW` dictionary on `globalThis.__DS_I18N_Locales_zhTW`, `utils/i18n.locales.en.js` registers the `en` dictionary on `globalThis.__DS_I18N_Locales_en`, and `utils/i18n.locales.js` (v4.11.14 split) aggregates them into `globalThis.__DS_I18N_Locales`. All three carry pure data and no logic.
- `utils/i18n.js` (v4.3.3) registers `window.dsI18n`, the core i18n engine with `setLocale()` and `t(key)` lookup — see the Language / Locale Switcher section below. The locale string maps live in `utils/i18n.locales.zhTW.js` and `utils/i18n.locales.en.js` (aggregated by `utils/i18n.locales.js`); the engine reads them off `__DS_I18N_Locales` synchronously at the top of its IIFE, with a `require('./i18n.locales.js')` fallback guarded by `typeof require !== 'undefined'` for the Node/vitest path. **The browser has no such fallback**, so every loader must place the three locale scripts (`i18n.locales.zhTW.js`, `i18n.locales.en.js`, `i18n.locales.js`) immediately before `i18n.js` or `zh_TW`/`en` resolve to undefined and every translated string breaks silently. Four loaders must stay in agreement: `manifest.json` (`content_scripts[0].js`), `popup/popup.html`, `popup/editor/editor.html`, and `test/setup/vitest.setup.js`.
...
- `popup.toast.js` (v4.11.10 split) registers the `Toast` key on that same `window.__DS_PopupModal` object. `popup.modal.js` holds only `Modal`, so the two unrelated components live in separate files per `coding-guidelines` §8. Both files self-mount via `Object.assign(window.__DS_PopupModal || {}, { … })` rather than a single object literal, so neither clobbers the other's key and the two are order-independent. `popup.js` destructures `const { Modal, Toast } = window.__DS_PopupModal;`. The manager bundles receive `Modal`/`Toast` through their `ctx` parameter, not the global.
...
On top of the open-time read (`StorageManager.syncNow()`, v4.7.0), `popup.live-sync.js` reflects changes made from another device, tab, or the standalone editor window while the popup stays open, by registering a single `chrome.storage.onChanged` listener, modeled on the equivalent listener already used by `content/content-script.js`.
