# UI Adjustment Module Architecture

> 📂 [DS studio Documentation](../) › [Architecture](../../ARCHITECTURE.md) (Chinese) › [Content Script Modules](CONTENT_SCRIPTS.md) › UI Adjustments
>
> **Related Specifications**: [UI Adjustment Specifications](../../spec/03-ui-adjustments.md) (Chinese)
...
- **Header selector**: `THINK_HEADER_TOGGLE_CLASS` (currently `._245c867`, derived as the first token of `THINK_HEADER_CLASS`) — the clickable toggle header inside the container. `hide-thinking.js` reads this value from the shared `ds-selectors.js` namespace. (Note: `hide-thinking.js` uses only this selector; `._5ab5d64` is used only in the unrelated `censor-reply-restore.thinkblock.js`.)
...
When neither the native button nor the wrapper can be located, `_injectButton()` no-ops and returns `false` — no button is created.
...
A route observer monitors `window.location.pathname` via a body `MutationObserver` + `popstate` event. On route change, the module resets all state (cancels active scroll, clears retry timers, removes the old button, stops all observers) and, after a 100ms DOM-settling delay, drives the gated retry loop `_tryConnectDom()`. This ensures the button reliably appears even when the input-area wrapper `.aaff8b8f` or the native button hasn't mounted yet — `_tryConnectDom()` retries every 500ms up to 120 times until readiness, and only then performs the injection, scroll-container reattachment, listener restart, and visibility evaluation. The 100ms settling timer's handle is stored in `_routeChangeTimer` (cleared before each re-arm, and by `disable()`), and `_tryConnectDom()` opens with a `if (!this.enabled) return;` guard. Both are load-bearing: without them a route change occurring within 100ms before `disable()` would still fire `_tryConnectDom()` after teardown had completed, re-injecting the button and restarting the wrapper observer with nothing left to tear them down again — `_stopWrapperObserver()` has already run and `_startWrapperObserver()`'s `if (this._wrapperObserver) return;` duplicate-guard sees a null field. The guard sits at the function entry rather than at each call site so it covers all three callers (the route timer, the `_enableRetryTimer` self-retry, and the synchronous call from `enable()`); this is why the retry callback carries no `enabled` check of its own.

### Visibility Logic

- **Show condition**: The first visible message's `getBoundingClientRect().bottom < 0` (scrolled above the viewport).
- **Hide condition**: `_isAtTop()` — true only on verifiable evidence: `scrollContainer.scrollTop <= 1`, or the **verifiable first-message anchor** (`[data-virtual-list-item-key="1"]`) is fully within the viewport. Loose "first mounted message" selectors are deliberately NOT trusted for the at-top verdict: with DeepSeek's virtual list the true first message is often unmounted, and any mounted message near the viewport top would otherwise yield a false "at top" (which would make `scrollToTopAndWait()` stop after ~one viewport).
...
The `MobileSidebarSwipe` module in `content/mobile-sidebar-swipe.js` detects bidirectional horizontal swipe gestures on mobile devices within the central 80% viewport area. Right-swipe (deltaX > 0) clicks the open button to show the sidebar; left-swipe (deltaX < 0) clicks the close button to hide it.
...
|-|-|-|-|
| a | `|deltaX| >= 50px` | `SWIPE_THRESHOLD_PX` | Minimum travel distance to filter noise (absolute value, symmetric in both directions) |
...
After passing all five conditions, `_onTouchEnd` branches on the sign of `deltaX`: positive (right-swipe) calls `_findButton()` for the open button; negative (left-swipe) calls `_findCloseButton()` for the close button.
...
`_findButton()` (open) uses a primary selector `div.ds-button--capsule.ds-button--iconLabelPrimary[role="button"]` with 5 fallback class combinations. `_findCloseButton()` uses a primary selector `div.ds-button--capsule.ds-button--iconLabelTertiary[role="button"]:has(path[fill-rule])` (the `:has` filter excludes the search button); when that misses, it takes the last of multiple matches for `div.ds-button--capsule.ds-button--iconLabelTertiary[role="button"]`, then tries `.ds-button--iconLabelTertiary.ds-button--icon:has(path[fill-rule])`. `_tryConnectDom()` checks once immediately, then polls every 500ms up to 60 times (≈30s) via the shared `content/retry-until.js` until the open button is found, then calls `_bindTouchEvents()`. The close button is looked up on demand at swipe time rather than cached at bind time.

### Master Switch Integration

`start()` delegates master-switch gating to `registerFeatureToggle` from `content/feature-toggle.js`, which obtains settings from background via `DSS_GET_SETTINGS` and subscribes to `DSS_SETTINGS_CHANGED` broadcasts. When the master switch is turned off, `disable()` unbinds touch listeners, cancels DOM polling, and resets swipe state. When turned back on, `enable()` restarts DOM polling.

### Lifecycle

| Method | Behavior |
|-|-|
| `start()` | Checks `isMobileDevice()`, then calls `registerFeatureToggle` with `onEnable` → `enable()` and `onDisable` → `disable()` |
| `enable()` | Guards on mobile + already-enabled, starts `_tryConnectDom()` |
| `disable()` | Unbinds touch events, cancels DOM polling, resets swipe state |
| `destroy()` | Calls `disable()`, then unregisters the feature toggle |
