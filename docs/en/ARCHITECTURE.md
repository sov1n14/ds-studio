...
- **Send button interception**: Listens for `pointerdown`, `mousedown`, and `click` at capture phase. The send button is identified by a two-tier strategy in `isSendButtonCandidate`:
  ...
  2. **Edit-window**: `isEditWindowSendButton` — the button carries both `ds-button--primary` + `ds-button--filled` variant classes (constant `EDIT_SEND_BUTTON_VARIANT_CLASSES`) AND a non-empty `span.ds-button__content` text label (no SVG).
  DeepSeek's "Stop Generating" button carries exactly the same classes as the send button and differs only in its SVG path (the stop icon path starts with `M2 4.88`), so the main composer send button is recognized by its icon path alone; the Stop button has no text label and does not match the edit-window tier either. The attachment button is excluded by both tiers: its classes are `ds-button--iconLabelPrimary` + `ds-button--capsule`, not `--primary`/`--filled`. After injection, the user's intended click is programmatically re-triggered via `requestAnimationFrame`.
