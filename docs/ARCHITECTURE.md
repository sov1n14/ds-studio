...
│   ├── censor-reply-restore.dom.js       ─  DOM orchestration entry bundle（stub，已合併至 censor-reply-restore.js）
│   ├── censor-reply-restore.dom.extract.js ─  Fragment extraction from DOM（stub，已合併至 censor-reply-restore.dom.resolve.js）
...
│   ├── go-top.locate.scroll.js  ─  Scroll-container locator bundle（stub，已合併至 go-top.locate.js）
│   ├── go-top.locate.anchor.js  ─  Anchor-element locator bundle（stub，已合併至 go-top.locate.js）
│   ├── go-top.render.js     ─  Button render orchestration bundle（stub，已合併至 go-top.js）
│   ├── go-top.render.button.js  ─  Button element creation bundle（stub，已合併至 go-top.render.combined.js）
│   ├── go-top.render.combined.js ─  go-top 按鈕 render 合併模組（button、inject、observer）
│   ├── go-top.render.inject.js  ─  Button DOM injection bundle（stub，已合併至 go-top.render.combined.js）
│   ├── go-top.render.observer.js ─  Render-related observer bundle（stub，已合併至 go-top.render.combined.js）
...
- **Send button interception**: Listens for `pointerdown`, `mousedown`, and `click` at capture phase. The send button is identified by its SVG send-arrow icon (`svg path[d^="M8.3125"]`, constant `SEND_BUTTON_ICON_SELECTOR`) covering both desktop and mobile layouts; the edit-window send button is identified by `ds-button--primary` + `ds-button--filled` via `isEditWindowSendButton`. After injection, the user's intended click is programmatically re-triggered via `requestAnimationFrame`.
...
- **持久化觀察記錄（v4.33.2）**：`recordLeaseObservation` 將每個 UUID 的觀察以 `{ lastActiveAt, observedAt }` 寫入 `chrome.storage.local`（鍵 `dss-last-seen-change:<uuid>`），補救掃描時比較 `lastActiveAt` 現值決定是否沿用已儲存的 `observedAt`，使租約到期判定在 service worker 冷啟動後仍正確。`lastActiveAt` 為 0 立即視為過期。掃描後清理 UUID 已不在佇列中的孤兒觀察鍵。
