...
- **雙欄版面**（v3.0.0 改版）：彈出選單使用左右並排的雙欄 flex 版面（`body` 寬度 660px）。左欄由上而下為：標題列卡片（標題 + 主開關）、Global Prompt 卡片（「全域預設提示詞」標籤 + 鉛筆編輯按鈕 + 專屬注入開關）、Prompt Group 卡片（提示詞組選取器 + `+` 按鈕 + 鉛筆編輯按鈕）、**備份與還原卡片**（含五項功能按鈕：`#exportJsonBtn` 匯出設定、`#importJsonBtn` 匯入設定、`#exportRestoredBtn` 匯出復原備份、`#importRestoredBtn` 匯入復原備份、`#clearRestoredBtn` 清除所有已還原紀錄）。右欄包含功能與匯出卡片、UI 調整卡片（側邊欄自動隱藏、隱藏思考過程、對話區域寬度、輸入框寬度、完整對話歷史面板）。
...
- **語言切換器**（v4.3.3）：功能與匯出卡片中的地球圖示按鈕（`#localeSwitcherBtn`）點擊後顯示語言面板（`#localePanel`），可切換繁體中文（zh_TW）與英文（en）。實作於 `popup/popup.locale.js`，使用 `utils/i18n.js` 的 `dsI18n.setLocale()` 進行切換，切換後重新載入彈出選單。
- **啟用/停用開關（主開關）**：控制提示詞注入功能是否啟用的開關。關閉時，所有子控制項（側邊欄自動隱藏、隱藏思考過程、注入系統時間、對話區域寬度、輸入框寬度、完整對話歷史面板）會透過 `disabled` 屬性停用。
- **思考過程開關**：控制匯出的 Markdown 是否包含 AI 思考過程的核取方塊。
- **參考連結開關**：控制匯出的 Markdown 是否包含引用參考連結的核取方塊。
- **系統時間開關**：控制是否在每則訊息開頭自動注入目前時間的核取方塊。
- **完整對話歷史面板開關**（v4.11.0）：UI 調整卡片中的 `#historyPanelToggle` 核取方塊，控制 `historyPanelEnabled`（預設啟用）。跨裝置／跨分頁變更透過 `popup.live-sync.js` 即時反映。受主開關連動（關閉主開關時一併停用）。
