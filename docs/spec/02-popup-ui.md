...
- **啟用/停用開關（主開關）**：控制提示詞注入功能是否啟用的開關。關閉時，所有子控制項（側邊欄自動隱藏、隱藏思考過程、防止自動回滾、注入系統時間、對話區域寬度、輸入框寬度）會透過 `disabled` 屬性停用。
