...
- **展開編輯區**：點擊編輯按鈕後，無條件移除所有 `.cc852ac5` 與 `._646a522` 元素的 `max-height`（以 inline style 設為 `none`），讓編輯區得以展開。此覆寫在離開編輯狀態後不還原。
- **非同步偵測**：編輯框 textarea 於點擊後才渲染，故以 `MutationObserver` 觀察訊息容器等待其出現（已存在時走同步路徑），硬性逾時 2000ms 後自動停止觀察。
