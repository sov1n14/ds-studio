...
- **重新命名流程**（v4.14.0）：點擊鉛筆按鈕（`#editPresetBtn`）開啟獨立編輯視窗，視窗標題處顯示**已聚焦並全選**的名稱輸入框（`#editorNameInput`），修改名稱與內容共用同一條自動儲存管道（500ms 防抖 + blur/關窗 flush）。重複名稱在儲存時被拒絕（`DUPLICATE_NAME`），儲存狀態區顯示紅色錯誤，直到名稱唯一才寫入。已開著的編輯視窗再次點擊鉛筆會重新載入其分頁，使名稱輸入框每次都重新聚焦。（v4.14.0 之前為：點擊下拉列內 `✎` 開啟預填當前名稱的內嵌對話框。）
