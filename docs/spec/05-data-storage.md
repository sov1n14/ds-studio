...
| `dsWebSearchToggle` | string | `'on'` | （v4.13.0，v4.17.0 改為二態）連網搜索按鈕的**進場預設值**：`'on'` 使頁面智能搜索按鈕在進場時為 `aria-pressed="true"`、`'off'` 為 `"false"`。每次頁面載入只套用一次，之後使用者手動切換的結果會保留；只在狀態不符時點擊。舊版 `'default'` 值已移除，讀取到殘留值一律當作 `'on'`（不寫回）。受主開關連動。 |
