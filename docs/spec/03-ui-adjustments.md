...
## 16. 自動展開訊息 (Auto Expand Messages) — v4.32.0
...
- **實作位置**：`content/auto-expand-messages.js`。`start()` 於模組載入時自動呼叫。

## 17. 隱藏思考過程 (Hide Thinking Process)

- **開關位置**：彈出選單「Features」卡片中的 `#hideThinkingToggle` 核取方塊，用於啟用/停用此功能（v4.32.0 自「UI 調整」卡片移至此處）。
...
- **開關位置**：彈出選單「Features」卡片中的單選群組（v4.32.0 自「UI 調整」卡片移至此處）（`input[name="websearchToggle"]`，兩選項 `開啟` / `關閉`，每個選項以 `.segmented-option` 包裹；圓形 radio 的外觀由 `popup-locale.css` 中 `:is(.locale-option, .segmented-option) input[type="radio"]` 的共用規則提供，與語言切換面板同一份樣式）。`開啟` 為標記中預先勾選者。
