...
Backslashes: inline `node -e '...'` and `sed` edits lose `\` (regex `\.` became `.`). When an edit needs a backslash, build it with `String.fromCharCode(92)` in node, then `sed -n` the line to confirm.

The limit is on the whole Bash command, not each heredoc: several `cat >>` heredocs chained in ONE Bash call (~140 lines total) still failed the same way; one ~60-line chunk per separate Bash call worked (reconfirmed 2026-09-24).
