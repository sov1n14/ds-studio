...
**How to apply:** Treat the implementation as complete for the two-tier/generic spec. Red was observed against HEAD via the [[project_red_phase_probe_vs_head]] probe. Next pipeline step is `test-executor` for the certifying green run; do not re-dispatch the implementer for this adjudication. Note the two legacy label-based preference cases ("picks the search toggle over an earlier deep-think toggle", "still picks the search toggle when it comes first") are obsolete under the new spec (no label tier) — the second one currently fails and needs an orchestrator decision.
