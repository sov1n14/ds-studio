...
**Why this differs from existing coverage:** `storage-manager.retry-sync-pull.spec.js`'s "does NOT push local PRESET_INDEX when cloud order is newer" test pends `dsLocalAuth: [PRESET_INDEX]` (not `PRESET_ORDER_META` itself) ??order-meta reconciliation there only happens via the trailing whole-store `resolveSyncConflict()` pass (see [[project_syncnow_unparked_push_ok]]), which apparently masks the missing per-key guard for the direct-pend case. Pending `dsPresetOrderMeta` itself is the uncovered path.
