...
**Why:** bare `python` is blocked by a hook (the `python-operations` skill demands `uv` + a project venv, which is overkill for a one-line edit); `node` is on PATH and unrestricted. Source files are a mix of CRLF (`popup/popup.js`, `utils/debounce.js`) and LF (`popup/popup.html`) with `core.autocrlf=true` and no `.gitattributes`, so a literal LF search string silently fails to match.
