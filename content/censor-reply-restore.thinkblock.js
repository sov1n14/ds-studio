    ...
    const bundle = {
        ...
        _buildThinkBlock(thinkFragment, elapsedSecs) {
            ...
            header.addEventListener('click', function () {
                ...
                // 切換箭頭 SVG 路徑
                const arrowIcon = container.querySelector(`.${selectors.THINK_HEADER_ICON_ROW_CLASS} > .ds-icon:not(.${selectors.THINK_ICON_CLASS}) svg path`);
                if (arrowIcon) {
            ...
            header.innerHTML = [
                `<div class="${selectors.THINK_HEADER_ICON_ROW_CLASS}">`,
                `<div class="ds-icon ${selectors.THINK_ICON_CLASS}" style="font-size: 16px; width: 16px; height: 16px;">`,
                ...
                '</svg></div>',
                `<span class="${selectors.THINK_STATUS_SELECTOR.slice(1)} ${selectors.THINK_TIME_CLASS}">` +
                ...
                '</svg></div>',
                '</div>',
                `<div class="${selectors.THINK_DIVIDER_CLASS}" style="opacity: 0;"></div>`
            ].join('');
            ...
            loadingDots.innerHTML = `<div class="${selectors.THINK_CONTENT_WRAPPER_CLASS} ${selectors.THINK_INNER_CONTENT_CLASS}"></div>`;
