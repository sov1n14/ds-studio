// 359 lines: three render concerns (button, inject, observer) for go-top unified per CA7
/**
 * DS studio — Go To Top Render Combined
 * 按鈕建構、注入策略、包裝容器 Observer 三合一。
 * 透過 Object.assign 合併至 GoToTop 物件。
 */
(function (root) {
    'use strict';
    ...
    const bundle = {
        ...
        // SVG path 資料逐字複製自 DeepSeek 原生箭頭圖示，朝上方向由 transform:scaleY(-1) 翻轉而來；此副本由本擴充功能自行持有，屬作者控制的穩定常量，不受上游選擇器輪換影響。
        _iconSvg() {
            return [
    ...
    // 發布合併 bundle 至全域，同時保留三個原始全域名以供向後相容
    root.__DS_GoToTop_render_combined = bundle;
